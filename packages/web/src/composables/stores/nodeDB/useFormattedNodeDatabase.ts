...
export const useFormattedNodeDatabase = createSharedComposable(() => {
    ...
    (ndb) => {
      ...
      for (const node of Object.values(ndb.nodeMap)) {
        ...
        const formatted: FormattedNode = {
          ...
          role: Protobuf.Config.Config_DeviceConfig_Role[node.user?.role ?? 0]?.replaceAll(
            '_',
            ' '
          ),
  ...
  function formatUptime(t?: number) {
    if (t) {
      return humanizeDuration(t * 1000, {
        round: true,
        serialComma: false,
        language: 'en',
        units: ['y', 'mo', 'w', 'd', 'h', 'm'],
  ...
  const fmt = (value: number | undefined, decimals: number, unit: string): string | null => {
    if (value === undefined || Number.isNaN(value)) return null;
    return `${value.toFixed(decimals)} ${unit}`;
  };

  const fmtInt = (value: number | undefined, unit: string): string | null => {
    if (value === undefined || Number.isNaN(value)) return null;
  ...
  const formatLocalStats = (
    m?: Protobuf.Telemetry.LocalStats
  ): FormattedLocalStatsMetrics | undefined => {
    if (!m) return undefined;
