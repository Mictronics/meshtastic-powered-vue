...
Settings forms can be smoke-tested in a headless browser without hardware (`npx playwright`; Chromium is usually already cached under `~/.cache/ms-playwright`), but that only exercises the default `create()`-empty config path. Bugs that only appear once a real device populates store state (e.g. reactive-proxy/`structuredClone` interactions) require an actual connected device to reproduce.
